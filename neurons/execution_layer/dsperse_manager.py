...
class DSperseManager:
    ...
    def on_incremental_slice_result(
        ...
        response_time_sec: float = 0.0,
        verification_time_sec: float = 0.0,
    ) -> tuple[bool, DSliceQueuedProofRequest | None, bool]:
        """
        Handle result for an incremental slice.

        Returns:
            Tuple of (is_run_complete, next_slice_request, is_duplicate)
        """
        task_id = (
            f"slice_{slice_num}" if not slice_num.startswith("slice_") else slice_num
        )

        result = self._incremental_runner.apply_result(
            run_uid=run_uid,
            task_id=task_id,
            success=success,
        ...
        if result == IncrementalRunner.RESULT_DUPLICATE:
        ...
        return False, (next_requests[0] if next_requests else None), False
    ...
    def on_incremental_tile_result(
        ...
        response_time_sec: float = 0.0,
        verification_time_sec: float = 0.0,
    ) -> tuple[bool, list[DSliceQueuedProofRequest], bool]:
        """
        Handle result for an incremental tile.

        Returns:
            Tuple of (is_run_complete, next_requests, is_duplicate)
        """
        result = self._incremental_runner.apply_result(
            run_uid=run_uid,
            task_id=task_id,
        ...
        return False, self.get_next_incremental_work(run_uid), False
